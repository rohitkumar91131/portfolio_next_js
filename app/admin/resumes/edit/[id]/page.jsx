...
export default function EditResume() {
  ...
  const [formData, setFormData] = useState(toResumeFormData({}));
  ...
  useEffect(() => {
    ...
    const fetchResume = async () => {
      try {
        ...
        if (data.success) {
          setFormData(toResumeFormData(data.data));
        } else {
  ...
  const requestOtp = async (actionType) => {
    ...
    if (!adminEmail) {
      setError(`Please confirm admin email to ${actionType} this resume.`);
      return;
    }
    setSaving(true);
    try {
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      if (pendingAction === "save") {
        const updateRes = await fetch(`/api/resume/${id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        ...
        const deleteRes = await fetch(`/api/resume/${id}`, { method: "DELETE" });
      ...
      setError(`Failed to ${pendingAction} resume`);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-black">
      ...
      <main className="mx-auto max-w-2xl px-6 py-10 pb-24">
        ...
        <h1 className="text-2xl font-bold mb-8 text-gray-900 dark:text-white">Edit Resume</h1>
